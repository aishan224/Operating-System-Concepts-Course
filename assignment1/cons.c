...
void *consumer(params *params){
    ...
    while(1){
        ...
        sem_wait(params->full);      //let the number of full -= 1
        params->count += 1;

        printf("procID is [%d] | threadID is [%d] | value is [%d]\n", pid, tid, params->ptr[params->current]);

        params->current = (params->current + 1) % 20;
...
int main(int argc, char **argv){
    ...
    /*However in consumer process, it's no use to call ftruncate*/
    ...
    addr = mmap(0, SIZE, PROT_READ, MAP_SHARED, shm_fd, 0);
    if (addr == MAP_FAILED){
        printf("map failed!\n");
        return -1;
    }
    ...
    for (i = 0; i < 3; i++){
        ...
        pthread_create(&tid[i], &attr[i], (void *)consumer, param);
    }
